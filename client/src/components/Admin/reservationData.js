...
const ReservationTable = () => {
  ...
  const ExpandedComponent = ({ data }) => (
    <div style={{ padding: '10px', fontSize: '14px', background: '#f9f9f9' }}>
      ...
      <p>
        <strong>หมายเลขการจอง:</strong> {data.reservationNumber}
      </p>
      <p>
      ...
      <p>
        <strong>วันที่จอง:</strong> {formatBuddhistDate(data.createdAt)}
      </p>
      <p>
        <strong>ตำแหน่ง:</strong> {translateStatus(data.status)}
      </p>
      <p>
        <strong>ชื่อ-สกุล:</strong> {translatePrefix(data.prefix)} {data.name} {data.surname}
      </p>
      <p>
        <strong>โทรศัพท์:</strong> {data.telephone}
      </p>
      <p>
        <strong>อีเมล:</strong> {data.mail}
      </p>
      <p>
        <strong>โรงเรียน:</strong> {data.school}
      </p>
      <p>
        <strong>ขนาดโรงเรียน:</strong> {translateSchoolSize(data.schoolSize)}
      </p>
      <p>
        <strong>ช่วงชั้น:</strong> {data.studentRange}
      </p>
      <p>
        <strong>ระดับชั้น:</strong>{' '}
        {data.studentRange ? (data.studentRange === 'มัธยม' ? 'ม.' : 'ป.') : ''}
        {data.studentLevel ?? '-'}
      </p>
      <p>
        <strong>จำนวนนักเรียน:</strong> {data.numberOfStudents}
      </p>
      <p>
        <strong>จำนวนวัน:</strong> {data.numberOfDays}
      </p>
      <p>
        <strong>ค่าบริการรวม:</strong> {data.price} บาท
      </p>
      <p>
        <strong>วันอบรม:</strong>{' '}
        {data.selectedDates?.map((d) => formatBuddhistDate(d)).join(', ') || '—'}
      </p>
      <p>
        <strong>ข้อมูลผู้ใช้งาน:</strong>{' '}
        {data.userInfo
          ? `${data.userInfo.username || 'N/A'} (${data.userInfo.email || 'N/A'})`
          : 'N/A'}
      </p>
      <p>
        <strong>รายวิชาที่เลือก:</strong>
      </p>
  ...
  return (
    <div style={{ padding: '10px' }}>
      ...
      />
      {isFetching && !isLoading && <span style={{ marginLeft: 8 }}>Refreshing…</span>}
    </div>
  );
};
